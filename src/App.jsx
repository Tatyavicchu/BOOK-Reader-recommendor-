...
import ImgText from './features/ImgText';
import FileText from './features/FileText';
import { VariablesProvider, Variables } from './components/variables';
import ExtractedText from './features/ExtractedText';

function App() {
  const [textdata, setTextdata] = useState('');
  const [submittedText, setSubmittedText] = useState(''); // New state for submitted text
  const [image, setImage] = useState([]);
  const [docs, setDocs] = useState('');

  const handleSubmit = () => {
    setSubmittedText(textdata); // Set the submitted text
    setTextdata('');            // Clear the input field after submission
    setImage([]);
    setDocs('');
  };
